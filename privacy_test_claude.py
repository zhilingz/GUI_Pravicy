...
def parse_and_annotate(ai_output, image_path, output_dir, print_ocr=False, no_save_image=False):
    ...
    for line in lines:
        ...
        try:
            ...
            # 坐标转换
            x1 = x1 * img_width / 705
            y1 = y1 * img_height / 1567
            x2 = x2 * img_width / 705
            y2 = y2 * img_height / 1567
            print(f"绘制坐标: [({x1:.1f}, {y1:.1f}), ({x2:.1f}, {y2:.1f})] (实际像素)")
...
def process(directory_path, enable_ocr=True, start=1, end=None, model_name="openai/gpt-5-pro", print_ocr=False, no_save_image=False, no_save_json=False, output_name="privacy_test"):
    ...
    prompt_template = dedent("""\
        ...
        ### Task
        Given an app screenshot (dimensions: {img_width} pixels width × {img_height} pixels height) taken while the Agent is working toward its goal, and the Agent's goal and response above:
        ...
        <original content/description> | <risk level> | <category_number>(<Category Name>) | <coordinates>

        - <original content/description>: for textual items, the exact visible text from the screenshot (verbatim); for non-text items, a concise description of the visible content (e.g. "face of a person", "bank card image").
        - <risk level>: one of "high", "medium", "low", "none".
        - <category_number>(<Category Name>): one of 1–6 above, only for items with privacy risk (high/medium/low). For items with risk level "none", output "-" in this field.
        - <coordinates>: the precise coordinates of this item in the screenshot in ACTUAL PIXEL VALUES based on the image dimensions ({img_width} × {img_height} pixels), as a JSON object like:
        {{"x1":80, "y1":250, "x2":740, "y2":350}}
        where x1,y1 is the top-left corner and x2,y2 is the bottom-right corner. The coordinate values must be in actual pixels within the range of 0 to {img_width} for x coordinates and 0 to {img_height} for y coordinates.
...
if __name__ == "__main__":
    parser = create_argument_parser(description='隐私信息分析工具 (实际像素坐标版)')
    args = parser.parse_args()
    validate_and_print_args(args)
    process(args.directory, not args.no_ocr, args.start, args.end, args.model, args.print_ocr, args.no_save_image, args.no_save_json, "privacy_test_hw")
