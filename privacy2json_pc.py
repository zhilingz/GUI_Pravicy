...
def process_test_data(directory_path, model_name="openai/gpt-5-pro", start=1, end=None,
                      no_save_image=False, no_save_json=False, output_name="privacy2json_pc"):
    """
    处理测试数据目录
    ...
    prompt_template = dedent("""\
        ...
        ### Task
        Given a PC desktop screenshot taken while the Agent is working toward its goal, and the Agent's goal and response above:
...
if __name__ == "__main__":
    parser = create_argument_parser(description='测试数据隐私分析工具 (PC test_data格式)')
